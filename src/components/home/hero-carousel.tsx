...
export function HeroCarousel({ isAuthenticated = false }: HeroCarouselProps) {
  ...
  return (
    ...
    >
      {/* ───── Mobile layout: text-first, compact image below ───── */}
      <div className="lg:hidden">
        {/* Text content */}
        <div className="px-4 pt-5 pb-6">
          <Link to="/" aria-label="Flex My Stuff home">
            <img
          ...
          <p className="mb-3 inline-flex rounded-full border border-primary-foreground/25 bg-primary-foreground/10 px-3 py-1.5 text-[0.65rem] font-bold uppercase tracking-[0.16em] backdrop-blur-sm">
            {slides[active].label}
          </p>

          <h1 className="text-[2.5rem] font-extrabold leading-[0.95] tracking-[-0.04em] text-primary-foreground">
            Don&apos;t Buy It.
            <br />
            <span className="text-highlight">Flex It.</span>
          </h1>

          <p className="mt-4 max-w-md text-base font-semibold leading-7 text-primary-foreground/90">
            Borrow what you need. Earn from what you own.
          </p>

          <div className="mt-5 flex flex-wrap gap-2.5">
            <Button asChild size="default" variant="highlight" className="rounded-full px-6">
              <Link to="/explore">Find a Flex <ArrowRight /></Link>
            </Button>
            <Button asChild size="default" variant="secondary" className="rounded-full px-6">
              <Link to="/add-listing">Flex an Item</Link>
            </Button>
          </div>

          <div className="mt-5 flex flex-wrap gap-x-5 gap-y-2 text-sm font-bold text-primary-foreground/85">
            <span className="flex items-center gap-1.5"><CheckCircle2 className="size-4 text-highlight" />Verified neighbors</span>
            <span className="flex items-center gap-1.5"><ShieldCheck className="size-4 text-highlight" />Secure rentals</span>
          </div>
        </div>
        ...
        <div className="relative h-[40vh] min-h-[260px] overflow-hidden">
          ...
          {/* Slide dots */}
          <div className="absolute bottom-3 left-4 right-4 z-20 mx-auto flex max-w-7xl items-center">
            <div className="flex gap-2" role="tablist" aria-label="Choose hero image">
              {slides.map((slide, index) => (
                <Button key={slide.src} type="button" size="icon" variant="ghost" role="tab" aria-selected={index === active} aria-label={`Show slide ${index + 1}`} onClick={() => selectSlide(index)} className="size-9 rounded-full hover:bg-primary-foreground/15">
                  <span className={cn("h-1.5 rounded-full transition-all", index === active ? "w-6 bg-highlight" : "w-2.5 bg-primary-foreground/50")} />
                </Button>
              ))}
            </div>
          </div>
          ...
          <div className="pointer-events-none absolute inset-0 z-20 flex items-center justify-between px-2">
            <div className="pointer-events-auto">
              <Button type="button" size="icon" variant="secondary" className="size-9 rounded-full shadow-lg" aria-label="Previous image" onClick={() => selectSlide(active - 1)}><ArrowLeft className="size-4" /></Button>
            </div>
            <div className="pointer-events-auto">
              <Button type="button" size="icon" variant="secondary" className="size-9 rounded-full shadow-lg" aria-label="Next image" onClick={() => selectSlide(active + 1)}><ArrowRight className="size-4" /></Button>
            </div>
          </div>
        </div>
      </div>

      {/* ───── Desktop layout: full-bleed image with overlaid text ───── */}
      <div className="relative isolate hidden min-h-[640px] overflow-hidden lg:block lg:h-[calc(100svh-5rem)] lg:max-h-[860px]">
